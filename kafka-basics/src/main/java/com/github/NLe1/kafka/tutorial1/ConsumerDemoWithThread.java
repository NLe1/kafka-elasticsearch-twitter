...
public class ConsumerDemoWithThread {
    ...
    private void run() {
        ...
        Runtime.getRuntime().addShutdownHook(new Thread(() -> {
            ...
            try {
                ...
                System.out.println("Application is closing with exception " + e);
            }
            logger.info("Application has exited!");
        }));

        //wait until application is over
        try {
            latch.await();
        } catch (InterruptedException e) {
    ...
    public class ConsumerRunnable implements Runnable {
        ...
        public void run() {
            ...
            try {
                while (true) {
                    ConsumerRecords<String, String> records = this.consumer.poll(Duration.ofMillis(100)); //new in Kafka 2.0.0
